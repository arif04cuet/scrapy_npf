...
class SQLStorePipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if item['links']:
            for row in item['links']:
                ...
                if link['status'] == 404 or (link['isExternal'] and (row['firstLabel'].startswith('সরকারি') or row['firstLabel'].startswith('সরকার'))):
                    link['crawled'] = 1
    ...
    def close_spider(self, spider):
        ...
        for row in result:
            sql = "update tmp_links set status=200,crawled =1,hasData='%s' WHERE domain='%s' and link='%s' " % (
                row[2], row[0], row[1])
            self.cursor.execute(sql)
