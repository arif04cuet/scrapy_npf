        ...
        for firstLabel in response.css('#dawgdrops>ul>li'):
            for secondLabel in firstLabel.css('div>div'):
                for thirdLabel in secondLabel.css('ul>li'):
                    ...
                    item['firstLabel'] = firstLabel.css('a::attr(title)').extract_first()
                    item['secondLabel'] = secondLabel.css("h6::text").extract_first()
                    item['title'] = thirdLabel.css("a::attr(title)").extract_first()
                    item['link'] = thirdLabel.css("a::attr(href)").extract_first()
                    yield item
                    #items.append(item)
